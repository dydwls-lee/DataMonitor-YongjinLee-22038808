...
from data_monitor.aggregator import ORDER_STATUSES, Summary
...
def render_samples_table(samples: list[Sample]) -> str:
    if not samples:
        return _EMPTY_SAMPLES_MESSAGE

    header = f"{'ID':<8} {'이름':<20} {'평균생산시간(min/ea)':>20} {'수율':>8} {'재고':>8}"
    lines = ["=== 시료 목록 ===", header, "-" * len(header)]
    for sample in samples:
        lines.append(
            f"{sample.id:<8} {sample.name:<20} {sample.avg_production_time:>20} "
            f"{sample.yield_rate:>8} {sample.stock:>8}"
        )
    return "\n".join(lines)
