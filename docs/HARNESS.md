...
- 확인 항목: 상태별 집계(**결정 완료**: 이 도구는 이 모듈 자체 PRD.md대로 `REJECTED`를 포함해서 집계한다 — 통합 저장소 루트 PRD의 "모니터링 메뉴 REJECTED 제외" 규칙은 `console-mvc`에만 적용되며 이 도구에는 적용되지 않는다. 근거: 통합 저장소 [DECISIONS.md](../../../docs/DECISIONS.md)), 시료/주문 목록 조회, 재조회 시 최신 데이터 반영 여부, 원본 데이터 미변경 여부
